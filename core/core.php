...
function get_connection($user=0) {
	...
	$users_params = array(0=>array('u_guest', 'guest'),
						  1=>array('u_client', 'c'),
					 	  2=>array('u_datas', 'd'),
					 	  3=>array($db_user, $db_password));
	$conn = mysql_connect('localhost',$users_params[$user][0],$users_params[$user][1]) or die (mysql_error());
	mysql_select_db($db_base,$conn);
return $conn; 
}

function get_doc_params($doc_id, $param, $doctype='') {
/* функция для получения параметров объекта из доп. таблиц */
	global $conn;
	if (!(int)$doc_id) {return '';} //Ошибка параметров! ID должен быть цифрой!
	if (!$doctype) {
		$qry = mysql_query("select dp.`tablename` from `doctype` dp ".
							"where dp.`id`=(select dt.`type` ".
									"from `doctree` dt where dt.`id` = $doc_id)",$conn);
		$doc_type = mysql_fetch_assoc($qry);
		$doc_type = $doc_type['tablename'];
	}else {
		$doc_type = $doctype;
	}
	$qry = mysql_query("select $param from `$doc_type` where `id`=$doc_id",$conn);
	if (mysql_error()) {return '';}//Ошибка параметров! Данные не удалось получить!
 ...
 */
	...
	$params = array_merge($param,$url_params,$_GET);
//	$params = get_params();
