<? 
//распаковываем архив
$cron_path = "/home/sandafar/public_html";
include_once $cron_path.'/core/globalvars.php';

$zip = new ZipArchive;
$f1="ftp://".$ftp_user.":".$ftp_pass."@".$ftp_url."/price1.zip";
$f2=$cron_path.'/uploads/price1.zip';
copy($f1, $f2);
if ($zip->open($f2) === TRUE) {
	$zip->extractTo($cron_path."/uploads/");
} else {
	$to = $admin_email;
...
fclose($handle);
unlink($cron_path."/uploads/price.txt");
unlink($cron_path."/uploads/price1.zip");
